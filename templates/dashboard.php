...
if ($arrHostapdConf['BridgedEnable'] == 1) {
  ...
  $GLOBALS['gwconn']->run_exec_gateway('iw dev '.$client_iface.' station dump | grep -oE '.$MACPattern, $clients);
} else {
  $moreLink = "index.php?page=dhcpd_conf";
  $GLOBALS['gwconn']->run_exec_gateway('cat '.RASPI_DNSMASQ_LEASES.'| grep -E $(iw dev '.$client_iface.' station dump | grep -oE '.$MACPattern.' | paste -sd "|")', $clients);
}
